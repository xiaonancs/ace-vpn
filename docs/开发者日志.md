...
**结论：TUN 没有 IP 泄漏**——系统默认流量与显式代理均显示 `167.254.242.54 / loc=JP / colo=NRT`。
「北京」来自 App 的 GPS/基站或未走代理的国内 IP 归属，不是 VPS 真在北京。

**发现并修复的 TUN/DNS 问题**（影响稳定性/性能，不是 IP 泄漏）：

| 问题 | 修复 |
|------|------|
| `ipv6: true`（客户端）但 VPS 已禁 v6 | `mihomo.yaml` + `work/config.yaml` + override → `ipv6: false` |
| `tun.dns-hijack: []`（订阅模板覆盖） | override `ace-vpn-tuning.yaml` + 订阅后 patch → `any:53` |
| `mihomo.yaml` 含 `fake-ip-filter: "*"` + `respect-rules: false` | 删除 `"*"`，改 `respect-rules: true` |
| `auto-redirect: false` | 改 `true`（macOS 更好接管系统流量） |
| reality client `flow` 为空 | VPS x-ui.db 补 `xtls-rprx-vision` |

**VPS 同步加固**：
- 面板 `127.0.0.1:14285`（仅 SSH 隧道）
- UFW：`25500/tcp LIMIT` 防扫描爆破；对外仅 22/443/25500
- SSH `PasswordAuthentication no`
- IPv6 保持禁用

**订阅刷新后注意**：Party 重拉订阅会覆盖 `work/config.yaml` 的 tun 段；
`override/ace-vpn-tuning.yaml` 会在合并时补回 dns-hijack / ipv6:false。
