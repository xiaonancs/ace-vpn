...
### 9.6 把"零信任 / SaaS 公网域名"误当成"真·内网域名"

**现象（2026-04 极容易踩）**：把公司 SaaS 类 / 零信任网关后挂的服务域名
（如 `<sso>.<corp-office>.example` / `<biz-app>.<corp-app>.example` /
`<saas>.example`）加进了 `intranet.yaml` 的 `profiles.<>.domains`（强制
走公司内网 DNS）。结果一开 ace-vpn TUN：

- `dig <域名>` 返回 SERVFAIL（或假 IP）
- `curl` 解析失败/卡死
- 关掉 ace-vpn 反而能上

**根因**：这些域名其实是**公网域名**，公网 DNS 能查到公司零信任网关
（`<gw>.corp-a.example`）的国内公网 IP，鉴权由网关侧的企业 VPN 客户端
session 负责，并不需要走 `10.x.x.x` 内网 DNS。把它们加进
`profiles.<>.domains` 后，sub-converter 会自动给它们生成
`nameserver-policy → 公司 10.x DNS`，但你不在公司内网 / 企业 VPN 客户端
没真注入 10/8 路由（被 ace-vpn TUN 抢占全表后非常常见）→ DNS 查公司
DNS 必然 SERVFAIL。

**正确分类（intranet.yaml 顶部注释里也有）**：

| 类型 | 公网 DNS 解析 | 入口 IP | 应放在 |
|---|---|---|---|
| A. 真·内网（如 `<srv>.intranet`）| 解析不到 | 10/8 内网 | `profiles.<>.domains` |
| B. 公司 SaaS（如 `<saas-app>.example`）| 公网真实 IP | 公网 | `extra.cn` |
| C. 零信任网关（如 `<sso>.<corp-office>.example`）| 公网网关 IP | 公网网关 | `extra.cn` |

**判定方法**：彻底关掉所有 VPN（含企业 VPN 客户端），跑
`dig +short <域名>`。返回 IP（哪怕是网关 IP）→ B/C，放 `extra.cn`；空 /
SERVFAIL → A，放 `profiles.<>.domains`。

### 9.7 extra.cn 域名走默认 DoH 解析到海外 IP

**现象（2026-04，§9.6 修完后才暴露的进阶坑）**：`extra.cn` 里的零信任域
名虽然 schema 上对了，但 TUN 模式下：

- `dig` 拿到一个 fake-IP（198.18.x.x）
- `curl` Connected → TLS 握手卡死（10 秒超时）
- 通过 Mihomo HTTP 代理 CONNECT 这个域名，Mihomo 内部解析到的是
  该零信任网关的**海外节点 IP**（CDN 边缘节点）

**根因**：Mihomo 默认 `nameserver` 用 DoH（`doh.pub` /
`dns.alidns.com`），TUN + 海外 PROXY 节点开启时，DoH HTTPS 流量会经过
PROXY 节点出去 → **站在海外节点视角解析** → 返回零信任网关的海外节点
IP → DIRECT 直连这个海外 IP → 海外节点对未鉴权请求静默丢包/拒绝 → TLS
握手卡死。

**修复**：sub-converter 给 `extra.cn` 域名**强制走国内 UDP 公网 DNS**
（`119.29.29.29` DNSPod + `223.5.5.5` AliDNS），并把它们加进
`fake-ip-filter`。生成的 nameserver-policy：

```yaml
nameserver-policy:
  +.<your-zerotrust-domain>.example:
    - 119.29.29.29
    - 223.5.5.5
```

Mihomo 看到裸 IP 不需要预解析、直接 UDP 连出去（不会经 PROXY），永远拿到
国内视角的 IP，国内秒回 302。

**已固化在 sub-converter.py（`CN_PUBLIC_DNS` 常量）**，无需手工配。客户端
拉新订阅 + 清 fake-ip cache + 重启 mihomo 后即生效。

### 9.8 ace-vpn TUN 抢全表后企业 VPN 注入不进 10/8 路由

**现象**：企业 VPN 客户端打开后看似已连接，但路由表里**没有 10.0.0.0/8**
那条路由，所有内网请求实际打不通。

**根因**：macOS 上多个 VPN 客户端同时启动时，**后启动的那一个**经常无法
注入与已存在 utun 冲突的路由。ace-vpn TUN 用 `auto-route: true` 接管了
`0.0.0.0/0`（拆成 `1/8` `2/7` `4/6` ... 这种 CIDR 占满全表），企业 VPN
客户端启动后发现冲突就静默放弃注入 10/8 路由。

**修复**：调整启动顺序——先连企业 VPN 客户端，等 `netstat -rn -f inet |
grep "^10/"` 看到路由后，再开 ace-vpn TUN。两者基于"更具体路由优先"原则
共存：10.x 走企业 VPN，其他走 ace-vpn TUN。

```bash
# 验证企业 VPN 真的接管了 10/8（关键）
ifconfig | awk '/^utun/{i=$1} /inet 10\./{print i": "$2}'
netstat -rn -f inet | grep "^10/"
# 期望：utunX: 10.x.x.x  + 10/8 → utunX
```

只要企业 VPN 真注入了 10/8 路由，ace-vpn TUN 后启动也不会抢回来。
