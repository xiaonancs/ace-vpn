...
```
┌────────────────────────────────────────────────────────────┐
│ 配置层（Mac 本地 + 公私两仓库 + symlink）                   │
│   ace-vpn (public)  ace-vpn-private (private)              │
│     └─ private/intranet.yaml  →  symlink                   │
...
│   3x-ui  (web + sqlite store)   :<panel>/TCP + :2096/TCP   │
│   ace-vpn-sub  (Python HTTP)    :25500/TCP                 │
│     └─ /etc/ace-vpn/intranet.yaml （per-request reload）   │
│     └─ /etc/ace-vpn/intranet.yaml.backups/ （5 份滚动）    │
└─────────┬──────────────────────────────────────────────────┘
          │ HTTP (Clash YAML)        │ TLS (vless reality)
          ▼                          ▼
┌────────────────────────────────────────────────────────────┐
│ 客户端层                                                    │
│   Mihomo Party / Clash Verge   (Mac / Windows)             │
│   FlClash / CMFA               (Android)                   │
│   Stash / Shadowrocket         (iOS / iPad)                │
│     └─ override/ace-vpn-local.yaml   （Mac 单机本地池）     │
└────────────────────────────────────────────────────────────┘
```
...
Mac 改 intranet.yaml
   ...
   ▼
VPS  备份旧 intranet.yaml → backups/intranet-<时间戳>.yaml （保留 5 份）
     scp 覆盖 /etc/ace-vpn/intranet.yaml
     curl /healthz 自检   →  ok active_profiles=corp-a domains=4 cidrs=1
   │
   │  家人客户端"刷新订阅"
   ▼
GET /clash/<token>
     ├─ load_intranet_config()  （per-request parse，<1ms）
     ├─ build_rules()           （从 intranet + extra + 内置规则拼装）
     └─ 返回 Clash YAML  →  客户端 apply  →  全家生效
```
...
```
应用发起请求 https://host:port
     │
     ▼
DNS 解析
     │
     ▼ host 是否命中 fake-ip-filter？
   ┌─yes (内网/SaaS/lan)──→ nameserver-policy 指定 DNS → 真实 IP
   │
   └─no ───────────────→ 分配 198.18.x.x 假 IP
     │
     ▼
规则匹配引擎（顺序匹配，命中即停）
     │
     ▼ IP-CIDR 命中内网段 (10/8 etc)？
     │  yes → DIRECT（系统默认路由 → 公司 VPN 接管 10/8）
     │
     ▼ DOMAIN-SUFFIX 命中内网域名？
     │  yes → DIRECT
     │
     ▼ DOMAIN 命中海外白名单 (AI/SOCIAL/MEDIA)？
     │  yes → PROXY (VLESS to VPS)
     │
     ▼ GEOIP 命中 CN？
     │  yes → DIRECT
     │
...
应用 dig <host>
    │
    ▼
TUN 模式拦截所有 UDP 53  →  mihomo 内置 DNS 组件
    │
    ▼ host 命中 fake-ip-filter？
    │
    ├─ YES (跳过 fake-IP，走真实 DNS)
    │     │
    │     ▼ 命中 nameserver-policy？
    │     ├─ YES → 用 policy 指定的 DNS（dns_servers / CN_PUBLIC_DNS / 内网 DNS）
    │     └─ NO  → 用全局 nameserver
    │
    └─ NO  (返回 198.18.x.x fake-IP，等真正建 TCP 时反查 host 走规则)
```
...
```
[Mac iHome] 改 intranet.yaml
    │
    ├─→ git push (ace-vpn-private)
    │
    └─→ bash scripts/sync-intranet.sh --all-vps
            │
            ├─→ HostHatch    → backup(5 份滚动) → scp → /healthz
            └─→ Vultr 冷备   → backup(5 份滚动) → scp → /healthz

[Mac iWork] 想同步
    │
    └─→ git pull (ace-vpn-private)
            └─→ symlink 自动指向最新 intranet.yaml
            └─→ 不需要再 sync VPS（VPS 已是最新）

[家人客户端] 想拿到新规则
    │
    └─→ 客户端 GUI 点"更新订阅"
            └─→ GET /clash/<token> → sub-converter per-request reload
            └─→ 返回最新 Clash YAML → 客户端 apply → 10 秒生效
```
...
Mac 改 intranet.yaml
    ...
    ▼
sync-intranet.sh
    1. yaml.safe_load 本地校验
    2. ssh mkdir -p /etc/ace-vpn
    3. ssh 远端备份 → /etc/ace-vpn/backups/intranet-<ts>.yaml （留 5 份）
    4. scp intranet.yaml 覆盖
    5. curl /healthz 自检
    │
    │  下次订阅请求到达
    ▼
GET /clash/<token>
    │  per-request load_intranet_config()  （每次都重读 YAML）
    │  per-request build_rules()
    ▼
返回最新 Clash YAML
    │
    ▼
