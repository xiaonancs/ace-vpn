...
- 订阅里 `dns.fake-ip-filter` 写了 `+.ai.xiaomi.com` 等内网域名
- 订阅里 `dns.nameserver-policy` 写了内网 DNS（如 `10.234.253.8`）
- 客户端确实拉到了订阅、`profiles/xxx.yaml` 里内容正确
- **但** TUN 模式下 `dig live.ai.xiaomi.com +short` 仍返回 `198.18.x.x` 假 IP
- `dig @10.234.253.8` 也返回假 IP（TUN 拦截所有 UDP 53）
...
开着 `controlDns` 时，GUI 会把订阅的 `dns:` 段**整块替换**，换成 GUI 设置 → DNS 页里的默认配置，其中 `fake-ip-filter` 只含 `"*"`, `+.lan`, `+.local`, `+.market.xiaomi.com` 几条默认项——**订阅里塞进去的 `+.ai.xiaomi.com` 被直接丢弃**。
...
dig live.ai.xiaomi.com +short          # 应返回真实 10.x IP
```
