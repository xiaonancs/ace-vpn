...
>
> 事件型文档（每周做了什么 / 学到什么 / 踩了什么坑）请看 [开发者日志 dev-skill](./dev-skill.md)；本文专注"系统怎么搭起来 / 怎么用"。
...
sequenceDiagram
    ...
    Mac->>VPS: bash scripts/sync-intranet.sh [--all-vps]
...
→ 自研 `scripts/sub-converter.py`（~600 行 Python），原生支持 Reality + 规则硬编码 + intranet 热加载 + `extra.{cn,overseas}` prepend + `/match` 调试 + `/healthz`。
...
Clash Party / Mihomo Party 默认 `controlDns: true`，会把订阅的 `dns:` 段**整段替换**——服务端再怎么精心生成 `fake-ip-filter` / `nameserver-policy` 都会被静默丢弃。详见 [开发者日志 §4.A.1](./dev-skill.md#4a1-2026-04-19-mihomo-party-吞掉订阅的-dns-段-)。
...
[开发者日志 §4.A.6](./dev-skill.md#4a6-2026-04-24-把零信任--saas-公网域名误当成真内网域名-) 总结的 A/B/C 三类：
...
⚠️ **`fake-ip-filter` 写法是域名 / 后缀 list，不是 mihomo 文档里写的 `*`**——`*` 实际只匹配单段标签（如 `com`），等于没写。Mihomo Party 默认 `controlDns: true` 时塞的就是 `*` / `+.lan` / `+.local`，所以一开 controlDns 用户精心配的 fake-ip-filter 全被吃。详见 [开发者日志 §4.A.1](./dev-skill.md#4a1-2026-04-19-mihomo-party-吞掉订阅的-dns-段-)。
...
| 规则源 | 位置 | 谁能改 | 粒度 | 应用场景 |
|---|---|---|---|---|
| **A. sub-converter 内置硬编码** | `scripts/sub-converter.py` 的 `AI_DOMAINS` / `SOCIAL_PROXY` / `CHINA_DIRECT` 等常量 | 改完推 VPS | 全局 | "天底下所有人都该这么走"——AI / Google / YouTube / 抖音 / 淘宝 |
...
$EDITOR scripts/sub-converter.py
...
bash scripts/sync-intranet.sh [--all-vps]
# 家人客户端"刷新订阅"，10 秒生效
```
...
| **VPS intranet.yaml 滚动备份** | promote 推坏了想回退 | `sync-intranet.sh` 每次 scp 覆盖前，远端 `<dir>/backups/intranet-<时间戳>.yaml` 自动备份，**只保留最近 5 份**。回退：`scp <vps>:/etc/ace-vpn/backups/intranet-<时间戳>.yaml ./private/intranet.yaml && bash scripts/sync-intranet.sh` |
...
# Mac 端一行命令完整诊断
bash scripts/test-route.sh https://foo.example/
```
...
| 层 | 设备 | 角色 |
|---|---|---|
| **管理端** | iHome Mac / iWork Mac | 改 `intranet.yaml` / `local-rules.yaml` / 跑 sync-intranet.sh / promote-to-vps.sh |
...
flowchart TD
    subgraph S1["Mac iHome (改配置)"]
        ...
        M1 --> SY[bash scripts/sync-intranet.sh<br/>--all-vps]
    end
...
> 详细根因 / 定位步骤 / 修复方法见 [开发者日志 §4](./dev-skill.md#4-踩坑与填坑按主题分类)。本节只做速查表。

| 现象 | 大概率根因 | 跳到 |
|---|---|---|
| 内网域名拿到 `198.18.x.x` 假 IP | Mihomo Party `controlDns: true` 吞订阅 dns 段 | [§4.A.1](./dev-skill.md#4a1-2026-04-19-mihomo-party-吞掉订阅的-dns-段-) |
| 开 TUN `dig @10.x.x.1` 不响应 | TUN 拦截所有 UDP 53 | [§4.A.2](./dev-skill.md#4a2-2026-04-19-tun-模式拦截所有-udp-53) |
| 改完 `fake-ip-filter` 重启仍假 IP | fake-ip cache 落盘 | [§4.A.3](./dev-skill.md#4a3-2026-04-19-fake-ip-缓存持久化) |
| 国内 IP 走代理 | GeoIP mmdb 过期 | [§4.A.4](./dev-skill.md#4a4-2026-04-19-geoip-数据库过期) |
| `hosts:` 写死后规则失效 | hosts 在 DNS 之前生效，规则失效 | [§4.A.5](./dev-skill.md#4a5-2026-04-19-hosts-写死导致规则失效) |
| 一开 ace-vpn 公司 SaaS / 零信任不通 | 误把 B/C 类放进 `profiles.<>.domains` | [§4.A.6](./dev-skill.md#4a6-2026-04-24-把零信任--saas-公网域名误当成真内网域名-) |
| `extra.cn` 域名 TLS 卡死 | DoH 经海外 PROXY 解析到海外 CDN | [§4.A.7](./dev-skill.md#4a7-2026-04-24-extracn-域名走默认-doh-解析到海外-ip-) |
| 企业 VPN 看似已连但 10.x 不通 | ace-vpn TUN 抢全表后启动顺序错 | [§4.A.8](./dev-skill.md#4a8-2026-04-24-企业-vpn-客户端--ace-vpn-tun-启动顺序) |
| 订阅节点 `server: 127.0.0.1` | `UPSTREAM_BASE` 写本机 + 没设 `SERVER_OVERRIDE` | [§4.B.5](./dev-skill.md#4b5-2026-04-18-sub-converter-所有节点-server-127001) |
| HostHatch 下单被反欺诈 flag | IP 国家 vs 账单国家不一致 | [§4.B.7](./dev-skill.md#4b7-2026-04-21-hosthatch-下单被反欺诈-flag) |
| 改 sub-converter env 不生效 | 没显式 `systemctl restart` | [§4.C.2](./dev-skill.md#4c2-2026-04-18-改了-sub-converter-环境变量但不生效--新-token-返回-0-节点) |
...
- 开发者日志（每周新增功能 / 性能优化 / 踩坑）：[`docs/dev-skill.md`](./dev-skill.md)
