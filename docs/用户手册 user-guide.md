...
| 🔍 **一键诊断** | `bash scripts/test-route.sh <URL>` 输出规则命中 / DNS / TCP / TLS / TTFB 全链路 | [§9.1 换机同步](#91仅管理员换机同步拉两个仓库--symlink--pre-commit) |
...
```bash
bash scripts/list-rules.sh                # 看本机积累了什么
...
bash scripts/apply-local-overrides.sh     # 重新渲染，本机生效
```

### 7.4 攒一周之后批量推 VPS（让全家人都生效）

```bash
bash scripts/promote-to-vps.sh --dry-run  # 先看计划：会推什么
bash scripts/promote-to-vps.sh            # 真推（自动清空本地池）
```
...
| **③ 一键回退** | 出问题后 | `bash scripts/rollback-overrides.sh` 列备份选一个回退；或 `--last` 直接回退到最近一个；或 `--disable` 把整个本地 override 暂时关掉 |
...
修法：编辑 ~/workspace/publish/ace-vpn-private/local-rules.yaml 修正/删除上面那些坏规则，
     再跑一次 bash scripts/apply-local-overrides.sh
```
...
```bash
# 1) 列出所有备份（看历史，啥也不改）
bash scripts/rollback-overrides.sh --list

# 2) 一键回退到最近一个备份（最常用）
bash scripts/rollback-overrides.sh --last

# 3) 交互选：列出备份让你按编号选
bash scripts/rollback-overrides.sh
...
# 排查完后恢复：
bash scripts/rollback-overrides.sh --enable

# 5) 清空 override 内容（保留注册）—— 比 --disable 温和，等于"本地池为空"
bash scripts/rollback-overrides.sh --clear
```
...
1. **如果是刚加完规则**（管理员场景）：八成是规则把 profile 加载坏了。立刻跑 `bash scripts/rollback-overrides.sh --last`（30 秒内恢复），详见 [§9.4](#94仅管理员安全网应急回退别让一条坏规则把自己的网砍了)
...
2. 脚本读取 `private/env.sh` 里的 `VPS_NODES`，通常是 `hosthatch:<IP> vultr:<IP>`；
3. 本地 Mac 分别 SSH 到 HH / Vultr；
4. 在两台 VPS 上对同一批 URL 跑 `curl`；
5. 所有结果追加到本地日志 `~/Library/Logs/ace-vpn/vps-watch.log`；
6. 20 天后用 `scripts/vps-watch-summary.py` 汇总成功率、median、p95、平均耗时和每个 URL 的赢家。
...
```bash
export VPS_IP_HOSTHATCH="<HostHatch-IP>"
export VPS_IP_VULTR="<Vultr-IP>"
...
```bash
ssh -i "$VPS_SSH_KEY" "$VPS_SSH_USER@$VPS_IP_HOSTHATCH" 'echo hosthatch-ok'
ssh -i "$VPS_SSH_KEY" "$VPS_SSH_USER@$VPS_IP_VULTR" 'echo vultr-ok'
```

如果还要输密码，先给两台 VPS 装本机公钥：

```bash
ssh-copy-id -i ~/.ssh/id_ed25519.pub "$VPS_SSH_USER@$VPS_IP_HOSTHATCH"
ssh-copy-id -i ~/.ssh/id_ed25519.pub "$VPS_SSH_USER@$VPS_IP_VULTR"
```
...
```bash
python3 scripts/vps-watch-summary.py --all
```

如果日志在自定义路径：

```bash
python3 scripts/vps-watch-summary.py --log ~/Desktop/vps-watch.log
```
