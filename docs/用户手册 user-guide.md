...
### Q11：换了新设备后 TUN 突然用不了 / 出口变成国内 IP
...
- 脚本默认 `VPS_WATCH_DAYS=30`，第一次 `--log` 运行会记录开始时间，满 30 天后会自动 unload 这个 LaunchAgent。
...
默认运行 30 天后会自动停止。你也可以随时手动停止定时任务：
...
会。只要执行过 `launchctl load ...plist`，并且没有 `unload`，它就会每 30 分钟自动跑一次；默认满 30 天后自动停止。
...
基本不会。它每 30 分钟 SSH 到 `VPS_IP_LIST` 里的 VPS，各自 curl speed-test 列表里的 URL。主要消耗 VPS 出站和少量本地 CPU / 网络。

**Q：会不会产生很多日志？**

默认 30 天会自动停止；按当前 100 个以内 URL、2 台 VPS 估算，每天最多约 `48 × 2 × 100 = 9600` 行。TSV 文本体积仍较小，30 天通常也只是几十 MB 以内。
