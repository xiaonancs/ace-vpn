...
注意：这测的是 **VPS → 目标网站** 的出站质量，用来比较 HH / Vultr 哪台到 AI、YouTube、X、Discord、Telegram、Meta / Facebook / Instagram / WhatsApp、GitHub 等服务更稳。它不是完整的 `Mac → VPS → 目标网站` 代理链路测试。
...
长期运行时日志会持续增长；按当前约 50 个 URL、2 台 VPS 估算，每天约 `48 × 2 × 50 ≈ 4800` 行。TSV 文本体积仍较小，但建议按月归档或清理一次。
